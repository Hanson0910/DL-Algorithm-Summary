...
## 回归Loss
1. [L1 Loss, 不论对于什么样的输入值，都有着稳定的梯度，不会导致梯度爆炸问题，对离群点不敏感，具有较为稳健性的解]()
2. [L2 Loss, 各点都连续光滑，方便求导，而且梯度值也是动态变化的，能够快速的收敛，得到较为稳定的解]()
3. [smooth-l1 loss, 综合了L1和L2的优点，相比于L2损失函数，对离群点、异常值使用L1，梯度变化相对更小，训练时不容易跑飞。当预测值与真是差异很小时，使用L2损失，快速得到较为稳定的解]
4. [iou loss, 使用 IOU LOSS可以把回归框当成一个整体去优化]
5. [GIOU Loss,预测值和 gt 不重叠时也可以优化,可以分辨不同的重叠方式]
6. [DIOU Loss,直接最小化预测框与目标框之间的归一化距离，以达到更快的收敛速度。]
7. [CIOU Loss, 可以在DIOU Loss基础上回归宽高比差异]
# 优化篇
